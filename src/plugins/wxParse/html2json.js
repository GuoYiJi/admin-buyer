    ...
    HTMLParser(html, {
        start: function (tag, attrs, unary) {
            ...
            if (node.tag === 'font') {
                ...
                if (!node.attrs.style) node.attr.style = [];
                // if (!node.styleStr) node.styleStr = '';
                for (var key in styleAttrs) {
                    if (node.attr[key]) {
                        var value = key === 'size' ? fontSize[node.attr[key]-1] : node.attrs[key];
                        // node.attrs.style.push(styleAttrs[key]);
                        // node.attrs.style.push(value);
