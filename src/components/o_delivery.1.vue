        ...
        <div v-for="(itemzz,idzz) in item.orderGoods" :key="idzz">
         <img class="sPimg" :src="itemzz.image"/>
          <div class="textThad" >
            ...
            <!-- <span class="edit" @click="edit(itemss)">编辑</span> -->
          </div>
          <div v-for="(itemList,ids) in item.deliverList" :key="ids">
            <!-- <div class="maShuo">白色:{{itemzz.skuCode}}/{{itemzz.num}}件</div> -->
            <div class="maShuo">
              <span class="text">{{itemList.skuCode}}/{{itemList.num}}件</span>   
              <span class="edit" @click="edit(itemList)">编辑</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
    ...
    methods: {
        ...
        async edit(itemss) {
          ...
          const L_addOrderData = await this.$API.L_addOrder({
            // orderId: this.orderID,
          });
          console.log(L_addOrderData)
